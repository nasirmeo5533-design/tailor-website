...
import { scheduleCallSchema, parseFormErrors, FORM_MIN_SECONDS } from "@/lib/validation";
import { scheduleCallEmail, sendInquiry } from "@/lib/email";
...
export async function POST(request: Request) {
  ...
  const reference = makeReference("CL");
  const email = scheduleCallEmail(parsed.data);
  const { delivered } = await sendInquiry(email);
  track("schedule_call_submitted", { reference, date: parsed.data.date, time: parsed.data.time, delivered });

  return NextResponse.json({ ok: true, reference, delivered, date: parsed.data.date, time: parsed.data.time });
}
