...
import { bookingSchema, parseFormErrors, FORM_MIN_SECONDS } from "@/lib/validation";
import { bookingEmail, sendInquiry } from "@/lib/email";
...
export async function POST(request: Request) {
  ...
  const reference = makeReference("BRK");
  const email = bookingEmail(parsed.data);
  const { delivered } = await sendInquiry(email);
  track("booking_submitted", { reference, serviceSlug: parsed.data.serviceSlug, delivered });
